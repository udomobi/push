...
class EmailPop3Transport:
    ...
    def convert_header_to_unicode(self, header):
        ...
        try:
            return "".join([(_decode(bytestr, encoding)) for bytestr, encoding in email.header.decode_header(header)])
    ...
    def get_text(self, body):
        return self.get_body_from_message(self.get_email_object(body), "text", "plain").replace("=\n", "").strip()

    def get_html(self, body):
        return self.get_body_from_message(self.get_email_object(body), "text", "html").replace("=\n", "").strip()
    ...
    def process_incoming_message(self, message):
        ...
        if "Subject" in message:
            msg["subject"] = self.convert_header_to_unicode(message.get("Subject"))[0:255]
        ...
        if "To" in message:
            ...
            msg["to_header"] = self.convert_header_to_unicode(message.get("Delivered-To"))
    ...
    with r.lock("channel_mailbox_running", 3600):
        for channel in Channel.objects.filter(is_active=True, channel_type="EM"):
            ...
            if pop_hostname:
                ...
                try:
                    transport = EmailPop3Transport(
                        pop_hostname, channel.config["EMAIL_POP_PORT"], channel.config["EMAIL_USE_SSL"]
                    )
