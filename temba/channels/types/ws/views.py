...
class ClaimView(ClaimViewMixin, SmartFormView):
    ...
    class Form(ClaimViewMixin.Form):
        name = forms.CharField(label=_('WebSite Name'),
                               help_text=_("The WebSite Name"))

    form_class = Form

    def form_valid(self, form):
        org = self.request.user.get_org()
        self.object = Channel.create(org, self.request.user, None, 'WS',
                                     name=self.form.cleaned_data['name'], address=settings.WEBSOCKET_ADDRESS, schemes=[WS_SCHEME])
