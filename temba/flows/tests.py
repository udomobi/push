...
class ActionPackedTest(FlowFileTest):
    ...
    def test_email(self, in_flowserver):
        ...
        self.assertOutbox(
            0,
            "no-reply@temba.io",
            "Message from Trey Anastasio",
            "Hi there, my name is Trey Anastasio.",
            ["support@website.com"],
        )
    ...
    def test_update_contact(self, in_flowserver):
        ...
        self.start_flow()
        self.contact.refresh_from_db()
        self.assertEqual("Male", self.contact.get_field_value(ContactField.get_by_key(self.org, "gender")))
        ...
        self.contact.refresh_from_db()
        self.assertEqual(
            action["value"], self.contact.get_field_value(ContactField.get_by_key(self.org, "last_message"))
        )
    ...
    def test_add_phone_number(self, in_flowserver):
        ...
        self.create_msg(
            ...
            text="+12065551212",
            contact_urn=self.contact.urns.filter(path="+250788382382").first(),
        )
...
class ActionTest(TembaTest):
    ...
    def test_multilanguage_ussd_menu_partly_translated(self):
        ...
        ussd_ruleset.rules = [
            Rule(str(uuid4()), dict(base="All Responses"), menu_uuid, "R", TrueTest()).as_json(),
            Rule(
                str(uuid4()), dict(base="Test1"), None, "R", EqTest(test="1"), dict(eng="labelENG", hun="labelHUN")
            ).as_json(),
...
class FlowsTest(FlowFileTest):
        ...
        def assert_payload(payload, path_length, result_count, results):
            ...
            for key, value in results.items():
                ...
                if in_flowserver:
                    self.assertEqual(
                        results_keys,
                        {"name", "value", "category", "category_localized", "input", "node_uuid", "created_on"},
                    )
                else:
    ...
    def test_subflow(self, in_flowserver):
        ...
        parent.start(
            groups=[], contacts=[self.contact, self.create_contact("joe", "+12347778888")], restart_participants=True
        )
...
class TypeTest(TembaTest):
    ...
    def test_value_types(self, in_flowserver):
        ...
        self.assertTrue(
            Flow.find_and_handle(self.create_msg(contact=contact, direction=INCOMING, text="Born 23/06/1977"))
        )
        self.assertTrue(
