...
def change_quick_replies_format(ActionSet):
    actionsets = ActionSet.objects.filter(
        Q(actions__icontains="quick_replies") | Q(actions__icontains="url_buttons")
    ).order_by("created_on")
                ...
                if len(url_buttons_keys) > 0:
                    ...
                    for key in url_buttons_keys:
                        for index, resp in enumerate(url_buttons[key]):
                            _url_buttons.insert(index, {key: {"title": resp.get("title"), "url": resp.get("url")}})
